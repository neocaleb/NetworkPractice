...
data_matrix=scipy.io.mmread('mtx_example/matrix.mtx')
data_matrix=data_matrix.toarray()
data_table=pd.read_table('mtx_example/genes.tsv',sep='\t',header=None)
features=np.array(data_table)
gene_name=features[:,1]
data_table=pd.read_table('mtx_example/barcodes.tsv',sep='\t',header=None)
cell_id=np.array(data_table)
